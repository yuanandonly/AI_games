...
def display(valuesa, valuesb):
    ...
    line = '+'.join(['-'*(width*4)]*4)
    for a in numbers:
      print(''.join(valuesa[a+b].center(width)+('|' if b in '37B' else '') for b in numbers) + "  >  " + ''.join(valuesb[a+b].center(width)+('|' if b in '37B' else '') for b in numbers))
      if a in '37B':
        print(line + "  >  " + line)
...
def main():
  #file
  puzzles = readfile("puzzlesLarge.txt")
  for puzz in range(len(puzzles)):
    values = dict((c, '0123456789ABCDEF') for c in cells)
    puzzle = {}
